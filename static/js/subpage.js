...
// ── Fix modelViewerBlock URLs before frontend.js reads them ───────────────
// frontend.js validates modelUrl with new URL(src) which rejects relative paths.
// Run synchronously (before DOMContentLoaded) to rewrite relative wp-content/
// paths to absolute localhost URLs so frontend.js can initialize the viewer.
(function patchModelViewerUrls() {
  try {
    var origin = window.location.origin;
    var blocks = document.querySelectorAll('.modelViewerBlock[data-attributes]');
    for (var i = 0; i < blocks.length; i++) {
      var raw = blocks[i].getAttribute('data-attributes');
      if (!raw || raw.indexOf('wp-content') === -1) continue;
      // The attribute is HTML-entity-encoded. Replace relative wp-content paths
      // with absolute local origin paths so new URL() succeeds in frontend.js.
      // Pattern matches: &quot;wp-content\/  or  "wp-content/
      var updated = raw
        .replace(/(&quot;|")wp-content\\\//g, '$1' + origin + '/wp-content\\/')
        .replace(/(&quot;|")wp-content\//g,   '$1' + origin + '/wp-content/');
      if (updated !== raw) {
        blocks[i].setAttribute('data-attributes', updated);
      }
    }
  } catch (e) { /* silent */ }
})();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (
    ...
    async function transitionToFilter(url, shouldPushState, silent = false) {
    ...
    // Auto-refresh products grid from live site on page load so initial state is always current
    (function () {
      const activePill = modelFilters.querySelector('.filter-pill.active');
      if (activePill && activePill.dataset.url) {
        transitionToFilter(activePill.dataset.url, false, true);
      }
    })();
  ...
  if (
    ...
    async function transitionToBlogFilter(url, shouldPushState, silent = false) {
    ...
    // Auto-refresh blog grid from live site on page load so initial state is always current
    (function () {
      const activeBlogPill = blogFilters.querySelector('.filter-pill.active');
      if (activeBlogPill && activeBlogPill.dataset.url) {
        transitionToBlogFilter(activeBlogPill.dataset.url, false, true);
      }
    })();
  ...
  function normalizeVideoInput(videoInput) {
    ...
    // Relative file paths (e.g. wp-content/uploads/…/28.mp4) — resolve against current page
    if (/\.(mp4|webm|ogg|mov|m4v|glb|gltf|usdz)(?:$|[?#])/i.test(raw)) {
      try { return new URL(raw, window.location.href).toString(); } catch (e) {}
    }
  ...
  // ── Fallback 3-D viewer for product detail pages ───────────────────────
  // frontend.js (bp3d) renders modelViewerBlock via React. If it fails for
  // any reason we create a plain <model-viewer> ourselves.
  // We grab data-attributes NOW (synchronously) before frontend.js deletes
  // them with its 10 ms setTimeout.
  (function initProductModelViewerFallback() {
    const blocks = document.querySelectorAll(
      '.sd-text .modelViewerBlock[data-attributes], ' +
      '.woocommerce-product-details__short-description .modelViewerBlock[data-attributes]'
    );
    if (!blocks.length) return;

    // Save model data immediately while data-attributes still exists
    const saved = [];
    blocks.forEach(function(block) {
      try {
        var raw = block.getAttribute('data-attributes') || '';
        // Decode HTML entities then parse JSON
        var decoded = raw.replace(/&quot;/g, '"').replace(/&amp;/g, '&');
        var attrs = JSON.parse(decoded);
        var modelUrl = attrs && attrs.model && attrs.model.modelUrl;
        if (!modelUrl) return;
        var height = (attrs.styles && (attrs.styles.height || attrs.styles.height)) || '400px';
        saved.push({ block: block, modelUrl: modelUrl, height: height });
      } catch (e) { /* ignore */ }
    });

    if (!saved.length) return;

    // After frontend.js has had time to render (1.5 s), check each block
    setTimeout(function() {
      // Also fix any model-viewer elements on the page that still point to the
      // live server (CORS-blocked). Redirect them to localhost.
      document.querySelectorAll('model-viewer[src*="gamenic-virtual-studio.com/wp-content"]').forEach(function(mv) {
        var localSrc = mv.getAttribute('src').replace(
          /https?:\/\/gamenic-virtual-studio\.com(\/wp-content\/)/,
          window.location.origin + '$1'
        );
        mv.setAttribute('src', localSrc);
      });

      saved.forEach(function(item) {
        var block = item.block;
        // If bp3d/React already rendered a model-viewer, leave it
        if (block.querySelector('model-viewer')) return;
        // If React rendered a container but no viewer yet, still create fallback
        if (block.children.length > 2) return;

        // Ensure the <model-viewer> custom element script is available
        if (typeof customElements !== 'undefined' && !customElements.get('model-viewer')) {
          var s = document.createElement('script');
          s.type = 'module';
          s.src = 'js/model-viewer.latest.min.js';
          document.head.appendChild(s);
        }

        var mv = document.createElement('model-viewer');
        mv.setAttribute('src', item.modelUrl);
        mv.setAttribute('camera-controls', '');
        mv.setAttribute('auto-rotate', '');
        mv.setAttribute('ar', '');
        mv.setAttribute('loading', 'eager');
        mv.setAttribute('shadow-intensity', '1');
        mv.style.cssText = 'width:100%;height:' + item.height + ';background:transparent;display:block;';

        block.style.cssText = 'width:100%;height:' + item.height + ';display:block;';
        block.innerHTML = '';
        block.appendChild(mv);
      });
    }, 1500);
  })();
